...
extern int ch_verification;
extern int lastaccupdate;
extern void ch_verify();
/////a_teammate anticheat detection
#ifdef WIN32
//////////////debugger handling!/////////////////
...
{
#ifndef MINGW //not supported by gcc yet .. probably later compile it with vcc or use LibSEH
...
//	"bot", "Bot", "B0T", "b0t", 
//	"cheat","Cheat", "Ch34t",
//	"Hax", "H4X","H4x","Hax","hax", "hack","Hack","H4ck",
//	"Trainer",
//	"tr41n3r", "Tr4in3r", "Tr4iner", "Volatility", "volatility",
//};

	//char *encode_(char *input)
	//{
	...
	//	for(int i = 0; i < len; i++)
	//	{	
	//		int stringint = input[i]-17;
	//		char letter = char(stringint);
	//		output[i] = letter;
	//	}
	//	output[len] = '\0';
	//	return output;
	...
	char *decode_(char* input)
	{
		int len = strlen(input);
		char *output = new char[len+1];
		for(int i = 0; i < len; i++)
		{
			int chiffreint = input[i]+17;
			char letter = char(chiffreint);
			output[i] = letter;
		}
		output[len] = '\0';
		return output;
...
//	loopi(21) 
//	{
//		char *output = encode_(suspecious[i]);
//		conoutf(output);
//
//		char *input = decode_(output);
//		conoutf(input);
//	}
//}
//COMMAND(encrypt, "");

char *checkforexternalaimbot()
{

	PROCESSENTRY32 pe32 = { 0 };
	HANDLE hSnapshot    = NULL;
	char *klibstr = decode_("ZTa]T[\"!");
	defformatstring(kernlib) ("%s.dll", klibstr );
	HINSTANCE hDll = LoadLibrary(kernlib);
	delete klibstr;
	if(!hDll) return("not doable");
		...
		if (pProcess32First (hSnapshot, &pe32))
		{
			...
			do
			{
				char *susp[18] = 
				{
					...
					"7Pg","7#G","7#g","7Pg",
					"WPg","WPRZ","7PRZ","7#RZ",
					"CaPX]Ta","Ca#X]\"a","Ca#X]Ta",
					"E^[PcX[Xch",	"e^[PcX[Xch"
				};
				loopi(18)
				{
					char *cur = decode_(susp[i]);
					if( strstr(pe32.szExeFile, cur))
					{
						//conoutf("  %s  ", pe32.szExeFile);
						formatstring(extprog)("%s", pe32.szExeFile);
						delete cur;
						return extprog;
					}
				}
...
namespace game
{
	...
	const char festchiffre[] = "miovcyjvpidawsximhxgzbfsby}ct{asyzpn|idyktwsvihfuwdqweclojjyaycbd}pfmx}}yhv}uxfluegncjvpjeboexnix{wpfz|stgcpyrphv}nttht|xoczfvqnahjth}}l{xrmjgcwrruvrhdhwogitsujcodiyuwtcj}gzktmetxumiislsyukw|drfzytx{oxarflsrivxme{uayyhknlhdvpaebszrfcnw}eui{{sdccqcc}ubvdkzsdujiaxcalbn|tqwts}}kjkde";

	char *encode_string(char *input, const char *chiffre, int chiffrelen, int start)
	{
		int len = strlen(input);
		char *output = new char[len+1];
		...
		for(int i = 0; i < len; i++)
		{	
			int stringint = input[i];

			int chiffreint = chiffre[i]-97;
			//if(i == 3)conoutf("%d, %c|| %d, %c", stringint, (char) stringint, chiffreint, (char) chiffreint);
			int let = -1*(stringint - chiffreint);
			//if((let >= 9 && let <= 13) || let == 32) let *= -1; //bug wegen sscanf auf serverseite
	...
	char *decode_string(char* input, char chiffre[], int chiffrelen, int start)
	{
		int len = strlen(input);
		char *output = new char[len+1];
		...
		for(int i = 0; i < len; i++)
		{
			int stringint = -1 * input[i];
			//if((stringint <= -9 && stringint >= -13) || stringint == -32) stringint *= -1; //bug wegen sscanf auf serverseite
			int chiffreint = chiffre[i]-97;
			char letter = char(stringint + chiffreint);
	...
	{
		srand(totalmillis);
		while(!mysessionid) mysessionid = rand() % 900;
	}
	int getcn()
	{
        return player1->clientnum;
	}
	bool getoutputstring()
	{
#ifdef WIN32 
		if(debuggerrunning()) fatal(decode_("?[iYdbcS^]ccahC70C"));
#endif 
		//conoutf("debuggerrunning: %s", debuggerrunning() ? "Yes" : "No");
		loopi(210) leoutput[i] = '\0';
		char myoutput[210]; loopi(210) myoutput[i] = '\0';
		const ENetAddress *servaddress = connectedpeer();
		string hostname;
		char *extaim = checkforexternalaimbot();
		//md5file(); //checks for the mapmodels
		//const char *mapmodels = " ";//md5output(); //grabs the mapmodel output
		//int mapmodelzahl = mapmodelnum();
		//if(mapmodelzahl > 0 && strlen(getclientmap())) mapmodels = md5file();
		//else {
		//		mapmodels = (char *)malloc( strlen(getclientmap()));
		//		strcpy(mapmodels,getclientmap());
		//}
		if(mysessionid <= 0) randsessionid();
		if(player1->clientnum != -1 && servaddress && (enet_address_get_host_ip(servaddress, hostname, sizeof(hostname)) >= 0))
		{
			//strncpy(leoutput, player1->name, strlen(player1->name));
				formatstring(myoutput) ("%d%c\
%s%c\
%d%c\
%d%c\
%s%c\
%d%c\
%s%c\0", mysessionid, (char)42,  
		hostname, (char)42, 
		servaddress->port, (char)42,
		player1->clientnum, (char)42,
		player1->name, (char)42, 
		/*mapmodelzahl > 0 && strlen(getclientmap()) ? mapmodels : getclientmap()*/0, (char)42, 
		extaim, (char)42);
		}
		char *encodedmsg = encode_string(myoutput, festchiffre, strlen(festchiffre), 0);
		strcpy(leoutput, encodedmsg);
		strncat(leoutput, "\0\n", 2);
		delete encodedmsg;
		return strlen(leoutput) > 0;
	}
	////////////<-
	...
	void drawteammates(fpsent *d, float x, float y, float s)
	{
		...
		{
			fpsent *o = players[i];
			if(o != d && o->state == CS_DEAD)
			{
					settexture(isteam(o->team, player1->team) ? "packages/hud/blip_blue_dead.png" : "packages/hud/blip_red_dead.png");
					glBegin(GL_QUADS);
					drawteammate(d, x, y, s, o, scale);
					glEnd();
			}
	...
	void crashname() 
	{
		const char *filename = findfile("crashname.ini", "r");
        if(fileexists(filename, "r"))
		{	
			stream *nfile = openfile("crashname.ini", "r");
			if(!nfile)return;

			char buf[50];
			vector<char *> namen;
			nfile->getline(buf, sizeof(buf));
			...
			srand(totalmillis);
			int randomnumber = rand() % namen.length();
			switchname(namen[randomnumber]);
			loopv(namen) if(namen[i]) delete namen[i];
			namen.shrink(0);
			delete nfile;
		} else {
			conoutf("Crashname.ini missing");
		}
...
#include "collect.h"
	...
	VARP(confarbe,0,0,7);
	//VARP(teamconfarbe,0,1,7);
	VARP(mconfarbe,0,2,7);



	void toserver(char *text) { 
		if(game::ch_totalstats)if(strstr(text, "lol") || strstr(text, "LOL")|| strstr(text, ":D")|| strstr(text, "xD") || strstr(text, ":)")) game::statslog[0]++; 

		conoutf(CON_CHAT, "%s:\f0 %s", colorname(player1), text); addmsg(N_TEXT, "rcs", player1, text); 
	}
	COMMANDN(say, toserver, "C");

	void sayteam(char *text) {
		if(game::ch_totalstats)if(strstr(text, "lol") || strstr(text, "LOL")|| strstr(text, ":D")|| strstr(text, "xD") || strstr(text, ":)")) game::statslog[0]++; 
		conoutf(CON_TEAMCHAT, "%s:\f1 %s", colorname(player1), text); addmsg(N_SAYTEAM, "rcs", player1, text); }
	COMMAND(sayteam, "C");
	...
	void sendintro()
	{
		...
		sendclientpacket(p.finalize(), 1);
		if(ch_verification && multiplayer(false))
        {
                //ch_verify();
                lastaccupdate = totalmillis;//a_teammate acc
        }
	}
    ...
    ////////////a_teammate nooblounge voting
    enum { E_VOTING,            //starts a new voting
           E_VOTEOPTION,        //get maps and modes to vote
           E_VOTE,             //another player voted for a mapmode / or you vote for a mapmode and send it to the server
           }; 
    struct voteoption {
        string map;
        int mode;
        vector<fpsent *>votes;
        voteoption() { map[0] = mode = 0; }
    };
    vector<voteoption *> voteoptions;
    void clearvoting()
    {
        loopv(voteoptions) {
            voteoption *v = voteoptions[i];
			if(!v) continue;
            v->votes.shrink(0);
            delete[] v;
        }
        voteoptions.shrink(0);
    }
    ICOMMAND(getname, "", (), result(player1->name));
    ICOMMAND(getvotemode, "i", (int *number), { if(*number < voteoptions.length()) result(server::modename(voteoptions[*number]->mode));});
    ICOMMAND(getvotelen, "", (), intret(voteoptions.length()));
    ICOMMAND(getvotemap, "i", (int *number), { if(*number < voteoptions.length()) result(voteoptions[*number]->map);});
    void parseextramessages(ucharbuf &p)
    {
        int type;
        ...
        while(p.remaining()){
            ...
            switch(type)
            {
                case E_VOTING:
                    clearvoting();
                    execute(newstring("showgui voting"));
                    break;
                case E_VOTEOPTION:
                {
                    voteoption *v = new voteoption;
                    getstring(text, p);
                    filtertext(v->map, text, false, MAXNAMELEN*2);
                    v->mode = getint(p);
                    voteoptions.add(v);
                    //conoutf("voteoption %d: %s on %s", voteoptions.length(), server::modename(v->mode), v->map);
                    break;
                }
                case E_VOTE:
                {
                    ...
                    if(num >= voteoptions.length()) break;
                    voteoptions[num]->votes.add(a);
                }
                default: return;
            }
            }
    }
    ///a_teammate: crashzone-server - login and sharing
    //todo: merge with extramsg
    
    //enum { CM_LOGIN, CM_GETUSERS, CM_GETUSERINFO }; //, CM_GETMAP, CM_GETDEMO, GETSCREENSHOT
    //struct czfileinfo {
    //    string name;
    //    string author;
    //    int date;
    ...
    //    bool isfriend;
    //    czfileinfo maps[5];
    //    czfileinfo screenshots[5];
    //    czfileinfo demos[5];
    //};
    //vector<czuser *> users;
    //void sendlogin(char *user, char *password)
    //{
    //    packetbuf p(MAXTRANS);
    //    sendstring(user, p);
    //    sendstring(password, p); //later: md5!
    //    sendclientpacket(p.finalize(), 3);
    //}
    ////server:
    ////hat bei userinfo noch freundesliste dabei
    //void receivecrashmsg(ucharbuf &p)
    //{
    ...
    //            {
    //                czuser *us = users[i];
    //                sendstring(us->name, p);
    //                us->isfriend = getint(p) != 0;
    //                users.add(us);
    //                if(p.overread()) break; // return?
    //                n = getint(p);
    //            }
    //            break;
    //        }
    ...
    //}
    ////client:
    //void parsecrashmsg(ucharbuf &p)
    //{
    //    while(p.remaining()) switch(getint(p))
    //    {
    //        case CM_LOGIN: //server sends whether your loginattempt was correct
    //        {
    //            bool error = getint(p) != 0;
    //            if(!error)  execute(newstring("showgui crashzone"));
    //            else execute(newstring("showgui crashlogin"));  
    //            break;
    //        }
    //        case CM_GETUSERS: 
    //        {
    ...
    //            while(n != -1)
    //            {
    //                czuser *us = new czuser;
    //                getstring(us->name, p);
    //                filtertext(us->name, us->name, false); //necessary?
    //                us->isfriend = getint(p) != 0;
    //                users.add(us);
    //                if(p.overread()) break; // return?
    //                n = getint(p);
	...
	///////////
	vector <char *>lastdemoinfos; //a_teammate demoname
	bool currentlynoconout = false; //disable conout for listdemos while procedure is running
    ...
    vector<int> gamespeeds;
    
	struct timecolumn
	{
		int stamp; //what demomillis this was changed
		int type; //which type (N_TEXT, N_CONNECT..)
		int one, two, three, four;
		vec lastpos;
	};
	vector<timecolumn> demotable;
	void parsemessages(int cn, fpsent *d, ucharbuf &p)
	{
		...
		case N_PAUSEGAME:
			{
				...
				{
					//gamepaused = val; 
                    gamepaused = reversed ? !val : val;
					player1->attacking = false;
				}
                if(!demopacket && reversed) popcon(); 
				else if(a) conoutf("%s %s the game", colorname(a), val ? "paused" : "resumed"); 
		...
		case N_GAMESPEED: 
			{
				...
				if(!demopacket) {
                    if(!reversed) {
                        gamespeeds.add(gamespeed); //backup
                        gamespeed = val;
                    }
                    else { 
                        gamespeed = gamespeeds.last();
                        gamespeeds.drop(); //delete last
                    }
                }
				extern int slowmosp;
		...
		case N_CLIENT: //todo reverse!!!! wie?? gar nich??
			{
		...
		case N_SOUND:
			if(!d) return;
			if(!reversed) playsound(getint(p), &d->o);
			else getint(p);
			break;
		...
		case N_FORCEDEATH: //todo reversed: letzte position zurückbekomen und forcealive.
			{
				int cn = getint(p);
				...
				if(!d) break;
				if(reversed) {
					timecolumn &tc = demotable.last();
					if(tc.stamp != nextplayback) conoutf("something wrong in forcedeath timestamp");
					if(tc.type == -2) clientdisconnected(tc.one, false);//just delete the last connected if he arrived by newclient(cn);
					else // he just really got slayed
						
					demotable.drop(); //and move back in the timetable
				}
				if(d==player1)
				{
					if(editmode) toggleedit();
		...
		case N_ITEMLIST:
			{
				...
				{
					if(mapchanged) entities::setspawn(n, !reversed);//true);
					getint(p); // type
				}
				break;
			}
		...
		case N_INITCLIENT:            // another client either connected or changed name/team
			{   
				...
				if(!d)
				{
					conoutf("invalid cn sent .. zufall??");
					getstring(text, p);
					getstring(text, p);
					getint(p);
					break;
                ...
                if(reversed) //todo: namenswechsel nicht möglich! der existiert ja immer schon, wenns rückwärst läuft.
                {
                    //needclipboard--; disconnect(d);
                }
				...
				else                    // new client
				{
					...
					extern void getstatsofpl(fpsent *d);
					getstatsofpl(d);
				}
		...
		case N_SHOTFX:
			{
				...
				s->totalshots += guns[s->gunselect].damage*(s->quadmillis ? 4 : 1)*(s->gunselect==GUN_SG ? 20 : 1); //a_teammate updates all
				break;
			}
		...
		case N_DAMAGE:
			{
				...
				if(!actor->skill && actor != player1) actor->totaldamage += damage; //a_teammate update all players
		...
		case N_TEAMINFO:
			for(;;)
			{
				...
				conoutf("arrived %s %d", text, frags);
			}
			break;
		...
		case N_SENDDEMOLIST:
			{
				...
				else loopi(demos)
				{
					...
					if(currentlynoconout) {
						size_t textlen = min(strlen(text), size_t(259));
						char *newdemoinfo = (char *) malloc(textlen);
						strncpy(newdemoinfo, text, textlen); ///a_teammate demo name
						newdemoinfo[textlen-1] = '\0';
						lastdemoinfos.add(newdemoinfo);
					}
					else conoutf("%d. %s", i+1, text);
				}
				if(currentlynoconout)
					currentlynoconout = false; //a_teammate -> no conout procedure is over
				break;
			}
...
#undef PARSEMESSAGES
	...
	//	conoutf("type was %d", type);
		}
	}

	/////////
	VARP(ch_renamedemos, 0, 1, 1);
	...
	struct demoinfoname {
		char Monat[4],
			Tag[3], Stunde[3], Minute[3];
		char *mode, *map;
		char *list()
		{

			defformatstring(step) ("%s_%s_%s_%s_%s_%s", Tag, Monat, Stunde, Minute, mode, map);
			char *output = new char[strlen(step)+1];
			strncpy(output, step, strlen(step));
			output[strlen(step)] = '\0';
			//empty mode and map now:
			delete mode;
			delete map;
			return output;
		}
	} mdi; //mydemoinfo
	char *makename(char *input)
	{
		//Mon Aug 20 19:24:01 2012: instagib, reissen, 218.73kB
		char *pch;
		pch = strtok (input,",");
		char *pchmap;
		pchmap = strtok (NULL,",");
		int maplen = strlen(pchmap);
		mdi.map = new char[maplen+1];
		loopi(maplen) mdi.map[i] = pchmap[i];
		mdi.map[maplen] = '\0';
		...
		while(pos <= 8 && klpch != NULL)
		{
			...
			case 0: //Wochentag
			case 5: //sekunde
			case 6: break;//jahr //alle 3 ignorieren

			case 1: {
				loopi(3) mdi.Monat[i] = klpch[i];
				mdi.Monat[3] = '\0';
				break;
					}
			case 2: {
				loopi(2) mdi.Tag[i] = klpch[i];
				mdi.Tag[2] = '\0';
				break;
					}
			case 3: {
				loopi(2) mdi.Stunde[i] = klpch[i];
				mdi.Stunde[2] = '\0';
				break;
					}
			case 4: {
				loopi(2) mdi.Minute[i] = klpch[i];
				mdi.Minute[2] = '\0';
				break;
					}
			case 7: { //effic
				int len = strlen(klpch);
				mdi.mode = new char[len+1];
				loopi(len) mdi.mode[i] =  klpch[i]; 
				mdi.mode[len] = '\0';
				break;
					}
			case 8: { //z.b. 'ctf' oder 'protect' 
				defformatstring(nmode) ("%s %s", mdi.mode, klpch);
				int totallen = strlen(nmode);
				delete mdi.mode;
				mdi.mode = new char[totallen+1];
				strcpy(mdi.mode, nmode);
				mdi.mode[totallen] = '\0';
					}
			default: break;
			}
			//conoutf("%d: %s", pos, klpch);

			klpch = strtok(NULL, " :");
			pos++;
		}
		//conoutf("success, input = %s", input);
		pch = strtok(input,","); 
		//conoutf("pch = %s", pch);
		//strcpy(mdi.map, pch); 
		char *output = mdi.list();
	...
	void receivefile(packetbuf &p)
	{
		...
		case N_SENDDEMO:
			{
				//a_teammate demo name
				string fname;
				if(ch_renamedemos && lastdemoinfos.length()) {
					char *dmoinfo = makename(lastdemoinfos[(lastgetdemonum==-1||lastgetdemonum>lastdemoinfos.length() ? lastdemoinfos.length() : lastgetdemonum)-1]);	
					formatstring(fname)("%s.dmo", dmoinfo);
					delete dmoinfo;
				}
				...
				lastgetdemonum = -1; 
				loopv(lastdemoinfos) delete lastdemoinfos[i];
				lastdemoinfos.setsize(0);
				if(!demo) return;
	...
	void getdemo(int i)
	{
		...
		if(ch_renamedemos) {
			currentlynoconout = true; //not conouting demolist, just saving
			lastgetdemonum = i<=0 ? -1 : i; ///a_teammate name of demos

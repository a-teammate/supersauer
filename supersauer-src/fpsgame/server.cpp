...
bool cpartsundtimeup = true;
    ...
    void setupdemoplayback()
    {
        ...
        string msg;
        msg[0] = '\0';
        defformatstring(file)("%s", game::demospath(smapname));
		...
		demomillis = demostartmin*60000;

        cpartsundtimeup = true;
	...
	void updatetimenow()
	{
			sendf(-1, 1, "ri2", N_TIMEUP, max((gamelimit - int(demostartmin*60000))/1000, 1));
			pausegame(true);
	}
    ...
    void readdemo()
    {
        ...
        {
            int chan, len;
			if(nextplayback > demostartmin*60000 && cpartsundtimeup) { 
                updatetimenow();
                clearparticles(); 
                cpartsundtimeup = false;
            }
	...
	//for updating files and receiving patches:

	//int susarev; //supersauerrevision
	//string supersauerdesc;
	//struct filerev { //current revision of the files to easily update changed files
	//	int revision, size;
	//	uint crc;
	//	string name;
	//};
	//vector<filerev> sfrevs;
	//const char *revisionfile() { return "filerevision.ini"; }
	//
	//void addfile(const char *name, int revision)
	//{
	//	if(!name || revision <= 0) return;
	//	stream *f = openrawfile(name, "rb");
	//	if(!f) return;
	//	filerev &r = sfrevs.add();
	...
	//	delete f;
	//}
	//ICOMMAND(addfile, "si", (const char *n, int *rev), addfile(n, *rev));
	//
	//void setrevision(int i, const char *desc)
	//{
	//	if(!desc || i <= 0) return;
	//	susarev = i;
	//	copystring(supersauerdesc, desc);
	//}
	//ICOMMAND(setrevision, "is", (int *rev, const char *desc), setrevision(*rev, desc));
...
{	
	...
	int sortterm = 0;
	bool upsidedown;
	enum { SORT_FILE = 1, SORT_TYPE, SORT_INFO, SORT_MODE, SORT_MAP };
    
	struct demoinfo {
		string map, pathname,
			gametype, //clanwar or duel or mixed
			infoline; //"osh(10) vs vaq(5)" or "ade(112) vs Drunken(120)" or "most frags trixxer(85)"
		int mode, mastermode;
#ifdef MYDEBUG
	int lasttype;
#endif
		...
		vector<extplayer *> spectator, clients;
		demoinfo() {
			file = NULL;
		...
		extteam *newteam(char *team, int j, bool newone = true)
		{
			...
			if(!t) loopv(teams) if(teams[i] && teams[i]->name[0] && team && team[0] && !strcmp(teams[i]->name, team)) t = teams[i];
			if(!t && newone) {
				if(!team || !team[0]) t = teams.add(new extteam("good"));
		...
		void addhappening(int type, char *text, int cn)
		{
			...
			d->type = type; //N_TEXT,
							//N_DIED //text = victims name, cn = actor
							//N_SCOREFLAG //text = teamname, cn = who scored
			strcpy(d->text, text);
			events.add(d);
		}
	...
	void demoinfo::parsemsgs(int cn, extplayer *d, ucharbuf &p)
	{
		...
		case N_SERVINFO:  {                 // welcome messsage from the server
				getint(p); 
				getint(p);	//if(getint(p) != PROTOCOL_VERSION) conoutf("protocol error: your demo isnt supported by the current demo previewer");
				getint(p);	getint(p);
				getstring(text, p, sizeof(text));	getstring(text, p, sizeof(text));
				break;
			}
		case N_PAUSEGAME:{
				getint(p);
		...
		case N_GAMESPEED: getint(p); getint(p);	break;
		...
		case N_SOUND: if(d) getint(p);break;
		...
		case N_SAYTEAM: { //does that even happen in demos?
				getint(p);	
				getstring(text, p);	
				break;	}

		case N_MAPCHANGE: {
				getstring(text, p); //map
				filtertext(text, text, false);
				copystring(map, text);
				mode = getint(p);
				getint(p); //entities
		...
		case N_ITEMLIST:
				while(getint(p)>=0 && !p.overread())	getint(p); // type
				break;
		...
		case N_SWITCHNAME:
			...
			if(d)
			{
				filtertext(text, text, false, MAXNAMELEN);
				if(!text[0]) copystring(text, "unnamed");
				if(strcmp(text, d->name)) //einflickpotential: einmal gleicher name, NUL, ip -> server reicht durch ..
				{
					copystring(d->name, text, MAXNAMELEN+1);
				}
			}
		...
		case N_SWITCHMODEL:
			{
				int model = getint(p); //genauso, bloss schlechter für ip
				if(d)	d->playermodel = model;
				break;
			}

		case N_CDIS: getint(p);
				//clientdisc(getint(p)); //dont delete who was there
			break;
		...
		case N_SHOTFX:
			{
				...
				//s->totalshots += guns[s->gunselect].damage*(s->quadmillis ? 4 : 1)*(s->gunselect==GUN_SG ? 20 : 1); //todo
				break;
			}
		...
		case N_DAMAGE:
			{
				loopk(5) getint(p);
//				int tcn = getint(p),	acn = getint(p); getint(p); getint(p);  getint(p);
				//extplayer *target = getclient(tcn),	*actor = getclient(acn);
				//if(!target || !actor) break; //einflickpotential
				// actor->totaldamage += damage; todo
				break;
			}
		...
		case N_ITEMACC: getint(p), cn = getint(p);break;
	...
	// coop editing messages//do they even exist in demo??
		case N_EDITF: case N_EDITT: case N_EDITM: case N_FLIP: case N_COPY:
		case N_PASTE: case N_ROTATE: case N_REPLACE: case N_DELCUBE:
			{
				if(!d) return;
				getint(p); getint(p); getint(p); //o
				getint(p); getint(p); getint(p); //s
				getint(p); getint(p);
					getint(p); getint(p); getint(p), getint(p); //c
				getint(p);
		...
		case N_SENDDEMOLIST:{ //does that even happen in a demo?
				int demos = getint(p);
...
case N_BASEREGEN:
{
	getint(p); getint(p); getint(p); getint(p); getint(p);
	break;
}
...
case N_BASESCORE:
{
	...
	getstring(text, p);
	getint(p);
	//if(m_capture) capturemode.setscore(base, text, total); //todo
	break;
}
...
case N_DEPOSITTOKENS:
{
	...
	break;
		...
		case N_ANNOUNCE:getint(p);
				break;

		case N_NEWMAP:
			{
				getint(p); //does that even happen?
				break;
			}
		...
		default:
#ifdef MYDEBUG
			conoutf("illegal message: last one was %d, type = %d", lasttype, type);
#endif
			return;
		}
		}
	...
	void demoinfo::assignteams() 
	{
		loopv(clients) 
		{
			...
			if(d->state == CS_SPECTATOR)
			{
				...
				extteam *t = newteam(d->team, -1); //create team with this name
				if(t)t->players.add(d);
				else if(teams.length() && teams[0]) teams[0]->players.add(d);
			}
		}
		if(!m_check(mode, M_TEAM) && teams.length() > 1) //should not happen
		{
	...
	{
		if(isduel() || isclanwar()) return; //it is one 
		extplayer *mfrags = 0;
	...
	bool demoinfo::setupreading(char *filename)
	{
		...
		else if(file->read(&hdr, sizeof(demoheader))!=sizeof(demoheader) || memcmp(hdr.magic, DEMO_MAGIC, sizeof(hdr.magic)))
			formatstring(msg)("\"%s\" is not a demo file", fn);
		else
		{
			lilswap(&hdr.version, 2);
		...
		{
			DELETEP(file)
			//conoutf(msg);  //yes? no? maybe? leave it out for now
			return false;
		}

		if(file->read(&this->nextreading, sizeof(this->nextreading))!=sizeof(this->nextreading)) //eof
		{
			endreading();
			return false;
		}
	...
	void demoinfo::load(char *demoname)
	{
			...
			while(file)
			{
				...
				ENetPacket *packet = enet_packet_create(NULL, len+1, 0);
				if(!packet || file->read(packet->data+1, len)!=len)
				{
					if(packet) enet_packet_destroy(packet);
					endreading();
					break;
...
//			DELETEP( d )
	...
	char *showdemohappenings(g3d_gui *cgui, uint *bottom)
	{
		...
		for(int start = 0; start < d->events.length();)
		{
			...
			for(int j = start; j < end; j++)
			{
                ...
                if(numlines >= 20) { end = j; break; } //next tab every 20 lines ( the bottom needs space)
				...
				if(!demoeventtime) formatstring(timestamp) ("");
					...
					case N_SCOREFLAG: 
						{
							...
							if(cgui->buttonf("%s%s%s \f2scored for team %s%s", 0xFFFF80, NULL, NULL, timestamp, !strcmp(p->team,"good")? "\f1":"\f3", p->name, !strcmp(e->text, "good") ? "\f1" : "\f3", e->text)&G3D_UP) demostartmin = max(0.0f, float(e->time/6000)*0.1f-0.1f);
							numlines++;
							break;
						}
					case N_TEXT: 
						{
							...
							if(cgui->buttonf("%s\f0%s:  \f7%s",0xFFFF80, NULL, NULL, timestamp, p->name, e->text)&G3D_UP) demostartmin = max(0.0f, float(e->time/6000)*0.1f-0.1f);
							numlines++;
							break;
						}
					case N_DIED:
						{
							...
							if((!strcmp(p->name, e->text) ? cgui->buttonf("%s%s%s \f2suicided",     0xFFFF80, NULL, NULL, timestamp, !strcmp(p->team, "good") ? "\f1" : "\f3", p->name) :
															cgui->buttonf("%s%s%s \f7fragged \f2%s",0xFFFF80, NULL, NULL, timestamp, !strcmp(p->team, "good") ? "\f1" : "\f3", p->name, e->text ))&G3D_UP) demostartmin = max(0.0f, float(e->time/6000)*0.1f-0.1f);
							numlines++;
							break;
						}
				}
	...
	char *showdemoinfo(g3d_gui *cgui)
	{
		...
		cgui->pushlist();
			cgui->spring();
				cgui->text(server::modename(d->mode), 0xFFFF80);
				cgui->separator();
		...
		if(d->spectator.length())	{
			listspectators(cgui, d->spectator);
		}
		return NULL;
	}
	...
	{
		static const char *names[] = { "file",  "type ",	"info", "mode ", "map " };
		static const int struts[] =  { 40,       10,			24, 	13,     14};
	...
	bool demobrowserentry(g3d_gui *g, int i, char *type, char *info, char *filen, int mode, char *map)
	{
		switch(i)
		{
		case 0:
			if(g->button(filen, 0xffffdd)&G3D_UP) return true;
			break;
		case 1:
		...
		case 2:
			...
			break;
		case 3:
			if(g->buttonf("%s", 0xffffdd, NULL, NULL, server::modename(mode))&G3D_UP) return true;
			break;
		case 4:
	...
	{
		if(!d) return false;
		if(demofiltername[0]) { //in case you are looking for a player
			bool matchpl = false;
			char *pfil = klbuchstaben(demofiltername);
			loopv(d->clients) {
				if(!d->clients[i] || !d->clients[i]->name || !d->clients[i]->name[0]) continue;
				char *cname = klbuchstaben(d->clients[i]->name);
				if(strstr(cname, pfil)) { matchpl = true; break; }
			}
			if(!matchpl) {
				if(strstr(klbuchstaben(d->map), pfil)) matchpl = true; 
				if(strstr(server::modename(d->mode), pfil)) matchpl = true;
				
				if(!matchpl) return false; //we can break already since the first statement is false
			}
		}
		if(demofilterfrags)
		{
		...
		for(int start = 0; start < demoinfos.length();)
		{
			...
			loopi(10)
			{
				...
				for(int j = start; j < end; j++)
				{
					...
					if(demofilter(&d) && demobrowserentry(g, i, d.gametype, d.infoline, d.pathname, d.mode, d.map)) { curdemoinfo = &d; selectedit = true; }
				}

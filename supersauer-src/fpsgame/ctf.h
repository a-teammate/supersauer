...
#endif
{
    ...
    void scoreflag(fpsent *d, int relay, int relayversion, int goal, int goalversion, int goalspawn, int team, int score, int dflags)
    {
        ...
        if(flags.inrange(goal))
        {
			...
			if(m_ctf){
				if(relay >= 0 && flags.inrange(relay)) conoutf(CON_GAMEINFO, "%s scored for %s (in %.1f s)", teamcolorname(d), teamcolor("your team", ctfflagteam(team), "the enemy team"), float(lastmillis-flags[relay].runstart)/1000.0f);
		...
		else if(m_ctf || m_protect || m_hold) if(su_totalstats) statslog[STATS_FLAGS]++;
        d->flags = dflags;

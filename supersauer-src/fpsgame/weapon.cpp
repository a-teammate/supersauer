...
namespace game
{
    ...
    void hit(int damage, dynent *d, fpsent *at, const vec &vel, int gun, float info1, int info2 = 1)
    {
        if(at==player1 && d!=at)
        {
            ...
            lasthit = lastmillis;
			if(su_totalstats) statslog[STATS_DAMAGE] += damage;
        }
	...
	XVARP(su_shotcolorr, 0, 64, 255);
	XVARP(su_shotcolorg, 0, 64, 255);
	XVARP(su_shotcolorb, 0, 64, 255);
	//VARP(su_shotsize, 0, 6, 50);
    ...
    void shoteffects(int gun, const vec &from, const vec &to, fpsent *d, bool local, int id, int prevaction)     // create visual effect from a shot
    {
        ...
        switch(gun)
        {
            ...
            case GUN_RIFLE:
                particle_splash(PART_SPARK, 200, 250, to, 0xB49B4B, 0.24f);
				particle_trail(PART_SMOKE, 500, hudgunorigin(gun, from, to, d), to, (((su_shotcolorr * 256) + su_shotcolorg) * 256) + su_shotcolorb, 0.6f, 20);

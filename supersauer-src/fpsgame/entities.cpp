...
namespace entities
{
    ...
    void pickupeffects(int n, fpsent *d)
    {
		...
		if(d == player1) if(game::su_totalstats){ 
			if(type>=I_SHELLS && type<=I_CARTRIDGES) game::statslog[STATS_AMMO]++;
			else if(type==I_HEALTH) game::statslog[STATS_HEALTH]++;
			else if(type==I_BOOST) game::statslog[STATS_BOOST]++;
			else if(type==I_QUAD) game::statslog[STATS_QUAD]++;
		}

...
namespace game
{
	...
	void toserver(char *text) { 
		if(game::su_totalstats)if(strstr(text, "lol") || strstr(text, "LOL")|| strstr(text, ":D")|| strstr(text, "xD") || strstr(text, ":)")) game::statslog[STATS_SMILED]++; 			
	...
	const char *demospath( char *n)
	{
		...
		if(fileexists(findfile(name, ""), "")) return findfile(name, "");
		else {
			formatstring(name) ("%s.dmo", n);
			if(fileexists(findfile(name, ""), "")) return findfile(name, "");
		}
		return n;
	}
	...
	struct demoinfos 
	{
		enet_uint32 host;
		enet_uint16 port;
		bool forceupdate; //on mapchange -> true
		bool available;
		struct demo
		{
			bool downloaded; //todo check whether its already downloaded in a previous session
			char *plain, //how it got received
				*converted; //how it look good for renaming
			char Monat[4],
		...
		vector<demo *>dmos;
		demoinfos() : forceupdate(false), host(0), port(0), available(false) { }
		void cleardmos()
		{
		...
		void adddemo(char *input, int num)
		{
			...
			while(pos <= 8 && klpch != NULL)
			{
				...
				case 1: {
					loopi(3) d->Monat[i] = klpch[i];
					d->Monat[3] = '\0';
					break;
						}
				case 2: {
					loopi(2) d->Tag[i] = klpch[i];
					d->Tag[2] = '\0';
					break;
						}
				case 3: {
					loopi(2) d->Stunde[i] = klpch[i];
					d->Stunde[2] = '\0';
					break;
						}
				case 4: {
...
#define loopdemos(d, b) \
	...
	char *showserverdemos(g3d_gui *g)
	{
		...
		game::mdi.update();
		if(!mdi.available)
			g->text("This Server has disabled Demos", 0xFF9900, "exit", "This Server seems not to record demos.");
		else {
			int returni = -1, fgcolor = 0xFFFF80;
				...
				g->space(4);
					...
					g->text("time", fgcolor);
					string tstr;
					loopdemos(d, formatstring(tstr) ("%s:%s", d->Stunde, d->Minute); g->text(tstr, 0xFFFFFF, NULL, d->converted))
					g->poplist();
				g->space(4);
					g->pushlist();
					g->text("date", fgcolor);
					string dstr;
					loopdemos(d, formatstring(dstr) ("%s.%s", d->Tag, d->Monat);	g->text(dstr, 0xFFFFFF, NULL, d->converted))
					g->poplist();
				g->space(4);
					g->pushlist();
					g->text("", fgcolor);
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_INITCLIENT:            // another client either connected or changed name/team
            {
                ...
                else                    // new client
                {
                    ...
                    if(needclipboard >= 0) needclipboard++;
						preview::getstatsofpl(d->clientnum, d->teamkills, d->deaths, d->totaldamage, d->totalshots);
                }
            ...
            case N_SHOTFX:
            {
				...
				s->totalshots += guns[s->gunselect].damage*(s->quadmillis ? 4 : 1)*(s->gunselect==GUN_SG ? 20 : 1); //
                break;
            }
            ...
            case N_SENDDEMOLIST:
            {
                ...
                else loopi(demos)
                {
					...
					if(currentlynoconout)	currentlynoconout = false; // -> no conout procedure is over
                break;
            }
    ...
    void receivefile(packetbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            {
				string fname;
				if(su_renamedemos && mdi.dmos.length()) {
					...
					if(!exist) formatstring(fname)("%s.dmo", dmoinfo);
					delete dmoinfo;
					if(exist) {
    ...
    void getdemo(int i)
    {
		...
		if(su_renamedemos) {
			...
			lastgetdemonum = i<=0 ? -1 : i; ///name of demos
			mdi.update();
		}

...
bool settexture(const char *name, int clamp, bool mipmaps)
{
...
#define MARKER "_super_" //every modified skin is included this way: its name is <usualskin>_super_<name> eg. flag_super_bright.jpg
struct skininfo {
...
} skininfos[SKIN_MAX] =
{
	...
	{ "snoutx10k",		"dds", "lower_r",		"upper_r" ,			"red/md5.cfg" },
	...
	{ "snoutx10k",		"dds", "lower_b",		"upper_b" ,			"blue/md5.cfg" },
	...
	{ "flags/red",		"jpg", "skin",			NULL ,				"md2.cfg" },
	{ "flags/blue",		"jpg", "skin",			NULL ,				"md2.cfg" },
	{ "flags/neutral",	"jpg", "skin",			NULL ,				"md2.cfg" },
}; 
...
void rewritemodelcfg(int skin, int version)
{
	...
	while(f->getline(buf, 4000))
	{
		...
		loopk(2) {
			...
			if(!part) continue;
			pos = strstr(buf, part); //go to .."lower_r.png> blabla blabla"
			if(pos && strstr(buf, a.ext) ) //find our line: look for the modelfile + the extension
			{ 
				char *rest = strstr(pos, "."); //go to the next dot (just dont put them into the filenames, otherwise everythings fine ".png> blabla" 
				if(!rest) break;
				*pos = '\0';
				formatstring(line) ("%s%s_super_%s%s", buf, part, a.versions[version], rest);
			}
		}
		cf.add(newstring(line));
	}
	delete f;
	stream *w = openfile(cfg, "w");
...
void findsuperskins()
{
	...
	loopk(SKIN_MAX)
	{
		...
		loopv(files)
		{
				...
				formatstring(n) ("%s%s", a.part1, MARKER); //doesnt recognize the file when just the second part is available, but thats not really a bug, you just have to be complete
...
COMMAND(findsuperskins, "");
void setsuperskin(int model, int version)
{
	...
	enumerate(textures, Texture, tex, 
    {
		...
		{
			tex.name[texlen-4] = '\0';
			char *sp = strstr(tex.name, "_super_");
			if(sp) sp[0] = '\0';
			defformatstring(newt) ("%s_super_%s.%s", tex.name, v, ending);
			delete[] tex.name;
...
ICOMMAND(setsuperskin, "ii", (int *model, int *version), setsuperskin(*model, *version));

void loopsuperskins(int model)
{
	if(model < 0 || model >= SKIN_MAX) return;
	vector<char> buf;
	string version;
...
ICOMMAND(loopsuperskins, "i", (int *model), loopsuperskins(*model));
...
XVARP(jpgquality, 0, 80, 100);
XVARP(jpgoptimize, 0, 1, 1);
XVARP(jpgsubsampling, 0, jpge::H2V2, jpge::H2V2);
...
void saveimage(const char *filename, int format, ImageData &image, bool flip = false)
{
    switch(format)
    {
        ...
        case IMG_JPG: 
        {
            ...
            jpge::compress_image_to_jpeg_file(filename, flipped.w, flipped.h, 3, 
                                           flipped.data, params);
            break;              
        }
        default:
...
void screenshot(char *filename, bool justfilename, int dir)
{
    ...
    if(dir)
    {
        copystring(buf, dir == DIR_DUEL ? "duel/" : "clanwar/");
        const char *dir = findfile(buf, "w");
        if(!fileexists(dir, "w")) createdir(dir);
    }
	...
	else if(su_renamescreens)
	{
		...
		if(isconnected(false, true))formatstring(name) ("%d_%s_%s%d_%s%d.%s_%s_%s", timeinfo->tm_mday, months[timeinfo->tm_mon],timeinfo->tm_hour <10 ? "0" : "" , timeinfo->tm_hour, timeinfo->tm_min <10 ? "0" : "",  timeinfo->tm_min, server::modename(game::gamemode), game::getclientmap(), justfilename ? "" : filename);
	...
	int d = 2;
	while(fileexists(foundfile, "r"))//there exist already a file with this name!
	{
		string newname;
		int newlen = strlen(buf)-strlen(imageexts[format]) - (d>2 ? size_t(4) : 0) - (d>9 ? 1:0); //copy the former filename minus the extension e.g. " (2).jpg" will be cutted if it creates " (3).jpg"
		strncpy(newname, buf, newlen);
		newname[newlen] = '\0';
		defformatstring(newending) (" (%d)%s\0", d, imageexts[format]);
...
ICOMMAND(screenshot, "s", (char *fn), screenshot(fn, false, 0));

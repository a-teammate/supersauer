...
float circle[3];
XVARP(su_originalbg ,0,0,1);

namespace game {
    XVARP(su_totalstats, 0, 1, 1);
}
XVARP(su_savehistory, 0, 1, 1);
XVARP(su_autoversioncheck, 0, 1, 1);
extern void loadhistory();
extern void savehistory();
void su_quit()
{
	if(game::su_totalstats) game::writestats(); 
	if(su_savehistory) savehistory();
	irc::cleanup();
}
void su_startup()
{
	//extern void getversion();
 //   if(su_autoversioncheck) getversion();
	if(game::su_totalstats) game::loadstats();
	if(su_savehistory) loadhistory();
}
//bool shallrestart = false;
...
/*
#ifdef WIN32
#define binaryname	"bin64/supersauer.exe"
#define binaryold	"bin64/supersauer_old.exe"
#define binarynew	"bin64/supersauer_new.exe"

bool updatebinary()
{
	if(!fileexists(binarynew, "r")) return false;
    remove(binaryold);
	rename(binaryname, binaryold);
	rename(binarynew, binaryname);
...
int timer = 0;
float box_scale = 0;
bool boxspinning_left = 0;
XFVARP(su_guitime, 15, 45, 90);
void drehbox_an(bool left)
{
    if(left == 0) {
        //rechtsdrehung
        boxspinning_left = false;
        box_spinning -= (90/su_guitime);
    }
    else {
        boxspinning_left = true;
        box_spinning += (90/su_guitime); //bugfix: first step
    }
    if(boxdrehtsich == false) {
        boxdrehtsich = true;
    }
}
//Gui rotation isnt implemented yet, because of a precision bug when hitting the window.

void drehbox_ctr()
{
	if(!curtime) return; //indipendent of fps
    if(boxdrehtsich)
    {
        if( timer < su_guitime) timer++;
        else {
            boxdrehtsich = false;
            timer = 0;
        }

        if(!boxspinning_left)//is spinning anticlockwards (showgui)
        {
            box_spinning += (90/su_guitime); //boxspinning anticlockwards
        }
        else //is spinning left (backwards)
        {
            box_spinning -= (90/su_guitime);//spin the box clockwards
        }

        //slight bumpeffect of our box
        if( timer < (su_guitime/2))	 box_scale -= 0.015 * (45.0/su_guitime);
        if( timer > (su_guitime/2))	 box_scale += 0.015 * (45.0/su_guitime); //for every guitime, bump exactly the same
        if( timer == su_guitime) box_scale = 0;
    }
    else
    {
        //bugfix: if cleared menu before it finished the rotation the box doesnt fulfill a complete 90° rotation and also the gui stucks at the half
        if( box_spinning%90 )
        { //todo: same for guilinks...
            
				if(box_spinning%90 <= 44)  box_spinning -= (90/su_guitime);
				else box_spinning += (90/su_guitime);
        }
    }
}
XVARP(su_dunsttexture,0,0,1);
XSVARP(su_background,"data/background/cube-sauer.jpg");
float bgmove = 0;
XVARP(bluremphasis, 0, 4, 100);
XVARP(blurrepeat, 0, 3, 100);

XHVARP(su_backgroundcolorr, 0, 0, 0xFF);
XHVARP(su_backgroundcolorg, 0, 0, 0xFF);
XHVARP(su_backgroundcolorb, 0, 0, 0xFF);
void render3d_menu(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    if(!inbetweenframes && !force) return;
    stopsounds();

	glClearColor( float(su_backgroundcolorr)/256.0f, float(su_backgroundcolorg)/256.0f, float(su_backgroundcolorb)/256.0f, 1.0 );
    if(caption) copystring(bgroundtext, caption);
    else copystring(bgroundtext, "");
    copystring(bgroundimages, su_background);
    if(mapshot || mapname)
    {
        if(mapshot && mapshot!=notexture)
        {
            defformatstring(maps_b)("<blur:%d/%d/0>packages/base/%s.jpg", bluremphasis, blurrepeat, mapname);
            copystring(bgroundimages, maps_b);

        }

        if(mapname) copystring(bgroundmaps, mapname);
        else copystring(bgroundmaps, "");
    }
    if(!mainmenu) return; //will be overwritten by renderprogress anyways :)
    glEnable( GL_DEPTH_TEST );
    glMatrixMode( GL_PROJECTION );
    glLoadIdentity();
    glFrustum( -1.5, 1.5, -1.2, 1.2, 1.5, 15 );
    //glFrustum( -1, 1, -1, 1, 1.5, 6.5 ); // Die Near-Clipping-Plane
    // befindet sich in einer
    // Entfernung von 1.5 Einheiten
    // hat die Abmessungen 3.0 * 2.4
    // Einheiten.
	...
	if(su_dunsttexture) {
        ...
        settexture("data/load/dunsttexture.png");
        glBegin( GL_QUADS );
        glTexCoord2f(0,bgmove);glVertex3f( E.x, E.y, E.z);
        glTexCoord2f(0,bgmove+1.0);glVertex3f( D.x, D.y, D.z);
        glTexCoord2f(1,bgmove+1.0);glVertex3f( A.x, A.y, A.z);
        glTexCoord2f(1,bgmove);glVertex3f( H.x, H.y, H.z);

        glTexCoord2f(1,bgmove);glVertex3f( F.x, F.y, F.z);
        glTexCoord2f(0,bgmove);glVertex3f(  G.x, G.y, G.z);
        glTexCoord2f(0,bgmove+1.0);glVertex3f( B.x, B.y, B.z);
        glTexCoord2f(1,bgmove+1.0);glVertex3f( C.x, C.y, C.z);


        glTexCoord2f(1,bgmove);glVertex3f( E.x, E.y, E.z);
        glTexCoord2f(0,bgmove);glVertex3f( F.x, F.y, F.z);
        glTexCoord2f(0,bgmove+1.0);glVertex3f( C.x, C.y, C.z);
        glTexCoord2f(1,bgmove+1.0);glVertex3f( D.x, D.y, D.z);

        glTexCoord2f(0,bgmove+1.0);glVertex3f( A.x, A.y, A.z);
        glTexCoord2f(1,bgmove+1.0);glVertex3f( B.x, B.y, B.z);
        glTexCoord2f(1,bgmove);glVertex3f(  G.x,  G.y, G.z);
        glTexCoord2f(0,bgmove);glVertex3f( H.x, H.y, H.z);
        glEnd();
    ...
    //g3d_render(); 									//wichtig, naja normales cubescript-menu halt
    // gl_drawhud(w, h);
}


void renderbackgroundalt(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    if(!inbetweenframes && !force) return;
    ...
    loopi(restore ? 1 : 3)
    {
        ...
        if(!su_originalbg){//loadskin
			glEnable(GL_BLEND);
            if(force)
            {
                settexture(bgroundimages, 0, false);
				if(!su_dunsttexture) glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                glBegin(GL_QUADS);
                ...
                if(su_dunsttexture) 
                {
                    ...
                    settexture("data/load/dunsttexture.png");
                    glBegin(GL_QUADS);
                    glTexCoord2f(0,		bgmove);  glVertex2f(0, 0);
                    glTexCoord2f(1.0f,	bgmove);  glVertex2f(w, 0);
                    glTexCoord2f(1.0f,	bgmove+1.0f); glVertex2f(w, h);
                    glTexCoord2f(0,		bgmove+1.0f); glVertex2f(0, h);
                    glEnd();
                    bgmove -= 0.0007f;
                }
            }
		//	glEnable(GL_BLEND);
            ...
            if(mapshot || mapname){
                if(mapshot && mapshot!=notexture)
                {
                    defformatstring(maps_b)("packages/base/%s.jpg", mapname);
                    copystring(bgroundimages, maps_b);

                }
                else
                {
                    settexture(su_background, 0, false);
...
XVARP(su_mainmenu, 0, 1, 1);

void renderbackground(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    if(!su_mainmenu||su_originalbg) 
        renderbackgroundalt(caption, mapshot, mapname, mapinfo, restore, force);
    else render3d_menu(caption, mapshot, mapname, mapinfo, restore, force);
}
//<-
...
void renderprogress(float bar, const char *text, GLuint tex, bool background)   // also used during loading
{
    ...
    if(!su_originalbg) {
        ...
        if(!settexture(bgroundimages, 0, false))
            settexture("data/background/wall.jpg", 0, false);
        glBegin(GL_QUADS);
        glTexCoord2f(0, 0); glVertex2f(0, 0);
        glTexCoord2f(1, 0); glVertex2f(w, 0);
        glTexCoord2f(1, 1); glVertex2f(w, h);
        ...
        settexture("data/load/load_k.png");
        glPushMatrix();
        ...
        settexture("data/load/load_m.png");
        //settexture("data/laden-unten.png");
        glPushMatrix();
        glTranslatef(w/2, h/2, 0);
        ...
        settexture("data/load/load_g.png");
        //settexture("data/laden-oben.png");
        glPushMatrix();
        glTranslatef(w/2, h/2, 0);
        ...
        if(bgroundmaps)
        {
            ...
            text_bounds(bgroundmaps, wb, hb);
            int x = ((w/2)*1.2f)-wb/2,
                y = (h/2)*1.2f;

            draw_text(bgroundmaps, x, y-128);
            glPopMatrix();
        }

        settexture("data/load/loading_frame.png", 3);
        glBegin(GL_TRIANGLE_STRIP);
        ...
        if(bgroundtext)
        {
            ...
            text_bounds(bgroundtext, wb, hb);
            int x = ((w/2)*1.6f)-wb/2,
                y = (h/2)*1.6f;

            draw_text(bgroundtext, x, y-64);
            glPopMatrix();
            settexture("data/load/loading_bar.png", 3);
            glBegin(GL_QUADS);
...
XVARFP(su_mainmenugamma, 0, 1, 1, setgamma());
void setgamma();
...
void setmaingamma()
{
	if(!su_mainmenugamma) return;
    cleargamma();
	curgamma = 100;
}
void setgamma()
{
	if(mainmenu && su_mainmenugamma) { setmaingamma(); return; }
...
int main(int argc, char **argv)
{
	...
	execfile("supersettings.cfg", false);
    for(int i = 1; i<argc; i++)
    {
        if(argv[i][0]=='-') switch(argv[i][1])
        {
    ...
    for(;;)
    {
        ...
        if(game::su_totalstats) game::logtime();
        inbetweenframes = false;

...
XVARP(su_cursorcolr,0,255,255);
XVARP(su_cursorcolb,0,255,255);
XVARP(su_cursorcolg,0,255,255); 
XSVARFP(su_cursor, "data/cursor/default.png", cursor = NULL);
...
void drawcrosshair(int w, int h)
{
    ...
    if(windowhit)
    {
        if(!cursor) cursor = textureload(su_cursor, 3, true);
        crosshair = cursor;
        chsize = cursorsize*w/900.0f;
        g3d_cursorpos(cx, cy);
        glColor3ub(su_cursorcolr, su_cursorcolg, su_cursorcolb);
    }
    else
    { 
    ...
    if(windowhit) glColor3ub(su_cursorcolr, su_cursorcolg, su_cursorcolb); 
...
VARP(wallclock, 0, 1, 1);
VARP(wallclock24, 0, 1, 1);
...
namespace game {extern int su_selogo;};
...
void gl_drawhud(int w, int h)
{
    ...
    if(game::su_selogo) conw = int(w/conscale), conh = int(h/conscale), abovehud = h*2.5f - FONTH, limitgui = h*3 - FONTH;
    else conw = int(w/conscale), conh = int(h/conscale), abovehud = conh - FONTH, limitgui = abovehud;	
    ...
    if(mainmenu)
    {
        ...
        int tw, th;
        text_bounds(super_name, tw, th);
        glTranslatef(0, h*5, 0);
        glScalef(1.5, 1.5, 1);
        glTranslatef(0, -h*5, 0);
        draw_textf(super_name, 0, h*5.0f-th);
        glPopMatrix();
    }
    ...
    if(!hidehud && !mainmenu)
    {
        ...
        if(hidestats || (!editmode && !showeditstats))
        {
            ...
            if(game::su_selogo) limitgui = min(h*3-FONTH, int(h*3*game::abovegameplayhud(w, h)));	    
            else limitgui = abovehud = min(abovehud, int(conh*game::abovegameplayhud(w, h)));     //conplatz
        }

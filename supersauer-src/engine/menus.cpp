...
struct menu : g3d_callback
{
    ...
    menu() : name(NULL), header(NULL), contents(NULL), init(NULL), onclear(NULL), showtab(true), currenttab(0) {}

    void gui(g3d_gui &g, bool firstpass)
    {
        cgui = &g;
        cgui->start(menustart, 0.03f, showtab ? &currenttab : NULL);
...
extern void drehbox_an(bool left);
...
int cleargui(int n)
{
    ...
    if(!guistack.empty())	drehbox_an(true);
    return clear;
}
bool setcurrenttab(int n)
{
    if(guistack.empty()) return false;
    guistack.last()->currenttab = n;
    return true;
}
...
{
    if(cgui) cgui->title(name, GUI_TITLE_COLOR, NULL, desc[0] ? desc : NULL);
}
...
void guicheckbox(char *name, char *var, float *on, float *off, char *onchange, char *desc)
{
    ...
    if(cgui && cgui->button(name, GUI_BUTTON_COLOR, enabled ? "checkbox_on" : "checkbox_off", desc[0] ? desc : NULL)&G3D_UP)
    {
...
void guiradio(char *name, char *var, float *n, char *onchange, char *desc)
{
    ...
    if(cgui && cgui->button(name, GUI_BUTTON_COLOR, enabled ? "radio_on" : "radio_off", desc[0] ? desc : NULL)&G3D_UP)
    {
...
void guiplayers()
{
    extern char *showplayers(g3d_gui *cgui); //mod filter -> serverbrowser lists found players
    if(cgui) 
    {
        char *command = showplayers(cgui);
        if(command)
        {
            updatelater.add().schedule(command);
            if(shouldclearmenu) clearlater = true;
...
void guidemoextevents(uint *bottom)
{
	if(cgui)
	{
		char *command = preview::showdemohappenings(cgui, bottom);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
void guidemoextinfo()
{
	if(cgui)
	{
		char *command = preview::showdemoinfo(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
void guidemobrowser()
{
	if(cgui)
	{
		char *command = preview::demobrowser(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
void guiircnames()
{
	if(cgui)
	{
		char *command = irc::ircnames(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
COMMAND(guiircnames, "s");

void guiircbuffer()
{
	if(cgui)
	{
		char *command = irc::ircbuffer(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;
...
COMMAND(guiircbuffer, "s");

void guiirc()
{
	if(cgui)
	{
		char *command = irc::ircwindow(cgui);
		if(command)
		{
			updatelater.add().schedule(command);
			if(shouldclearmenu) clearlater = true;

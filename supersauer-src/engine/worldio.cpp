...
VARP(savebak, 0, 2, 2);
XVARP(su_editingrenamebak, 0, 1, 1);
...
void setmapfilenames(const char *fname, const char *cname = 0)
{
    ...
    else if(savebak==1) formatstring(bakname)("packages/%s.BAK", mapname);
...
struct mapnametitle { const char *mname, *maptitle; };
...
const char *getmaptitle(const char *mname)
{
    ...
    else 
    {
        int extra = 0;
        if(hdr.version <= 29) extra++; 
        if(d->read(&hdr.blendmap, sizeof(hdr) - (7+extra)*sizeof(int)) != int(sizeof(hdr) - (7+extra)*sizeof(int))) { delete d; return NULL; }
		loopi(hdr.numvars)
		{
...
bool load_world(const char *mname, const char *cname)        // still supports all map formats that have existed since the earliest cube betas!
{
    ...
    setgamma();
    conoutf("reading map took %.1f seconds",  (SDL_GetTicks()-loadingstart)/1000.0f);

...
void resetorigin(){
playorigin.x = playorigin.y = playorigin.z = 512;
}

void rec(char* c){
if(*c) {
	char buf[1024];
	sprintf(buf, "rec/%s.rec", c);
	DELETEP(recording);
	recording = openfile(buf, "w");
		if(!recording) {
			conoutf("Konnte Aufnahme nicht starten %s", c);
		} else {
			conoutf("Aufnahme %s...", c);
		}
	} else conoutf("kein Dateiname angegeben.");
}
void recstop(){
DELETEP(recording);
DELETEP(playing);
conoutf("Aufnahme Gestoppt ...");

}
int playmils;
void recstart(char* c){
	if(*c) {
		playorigin = lu;
		DELETEP(recording);
		char buf[1024];
		sprintf(buf, "rec/%s.rec", c);
		playing = openfile(buf, "r");
		playmils = totalmillis;
		if(playing) conoutf("Wiedergabe von %s...", c);
		else conoutf("Aufnahme \"%s\" wurde nicht gefunden", c);
	} else conoutf("Kein Dateiname angegeben.");
}

VAR(playmillis,10,100,60000);


void playonce(){
	if(totalmillis - playmils < (int64_t)playmillis) return; // limit playback speed to avoid lag
	playmils = totalmillis;

char buf[1024] = "";
playing->getline(buf, sizeof(buf));
char *t = strtok(buf, "\t\n ");
if(t && *t) {
	...
	//selinfo sel;
    sel.o.x = playorigin.x + strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.o.y = playorigin.y + strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.o.z = playorigin.z + strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
    sel.s.x = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.s.y = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.s.z = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
    sel.grid = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.orient = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
    sel.cx = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.cxs = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.cy = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
	sel.cys = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
    sel.corner = strtol(s, NULL, 0); s = strtok(NULL, "\t\n ");
    sel.o.z += sel.grid;
    ...
    switch(type)
    {
    case 37: dir = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); mode = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); mpeditface(dir, mode, sel, true); break;
    case 37 + EDIT_TEX: tex = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); allfaces = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); mpedittex(tex, allfaces, sel, true); break;
    case 37 + EDIT_MAT: mat = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); filter = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); mpeditmat(mat, filter, sel, true); break;
    case 37 + EDIT_FLIP: mpflip(sel, true); break;
	case 37 + EDIT_COPY: 
		mpcopy(localedit, sel, true); 
		break;
    case 37 + EDIT_PASTE: mppaste(localedit, sel, true); break;
    case 37 + EDIT_ROTATE: dir = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); mprotate(dir, sel, true); break;
    case 37 + EDIT_REPLACE: tex = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); newtex = strtol(s, NULL, 0); s = strtok(NULL, "\t\n "); mpreplacetex(tex, newtex, false, sel, true); break;
    case 37 + EDIT_DELCUBE: mpdelcube(sel, true); break;
    }
}
if(playing->end()) {
	DELETEP(playing);
	conoutf("Finished playing.");
	}
}

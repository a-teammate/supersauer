...
XVARP(su_skinalpha,0,70,100);
XVARP(su_clearfields, 0, 1, 1);
XSVARP(su_iconsdir, "");

XVARP(su_howercolorr, 0, 0xFF, 0xFF);
XVARP(su_howercolorg, 0, 0x0A, 0xFF);
XVARP(su_howercolorb, 0, 0x00, 0xFF);

XVARP(su_howerinfo, 0, 1, 1);
XFVARP(su_howerinfodelay, 0, 1.0f, 10.0f);
XVARP(su_howerinfoscale, 3, 5, 10);
...
struct gui : g3d_gui
{
	...
	enum { SU_ROUNDED = 1, SU_GREEN = 2, SU_WARNING = 4, SU_ABORT = 8};    
    char *field(const char *name, int color, int length, int height, const char *initval, int initmode)
    {
        return field_(name, color, length, height, initval, initmode, FIELDEDIT, SU_ROUNDED);
    }

    char *keyfield(const char *name, int color, int length, int height, const char *initval, int initmode)
    {
        return field_(name, color, length, height, initval, initmode, FIELDKEY, SU_ROUNDED);
    }
    ////frame

	char *su_field(const char *name, int color, int length, int height, const char *initval, int initmode, bool warn)
    {
		int tags = (SU_ROUNDED|SU_GREEN);
		if(warn) tags |= SU_WARNING;
        return field_(name, color, length, height, initval, initmode, FIELDSHOW, tags);
    }
 #define QSIZE 30

    char *field_(const char *name, int color, int length, int height, const char *initval, int initmode, int fieldtype = FIELDEDIT, int tags = 0) //su_c -> su_ clients on the masterlist have coloured outline
    {	
        ...
        if(visible() && !layoutpass)
        {
			...
			if(su_clearfields) 
			{
				if(strlen(e->currentline().text) && !(tags&SU_ABORT || tags&SU_WARNING) && fieldtype==FIELDEDIT && su_clearfields) tags |= SU_ABORT;
	            bool iconhit = ishit(QSIZE, QSIZE, curx+w-QSIZE);
	            if(iconhit && mousebuttons&G3D_DOWN && tags&SU_ABORT)
	            {
         ...
         //   if(framed) glColor3ub(255, 0, 255);
            
            rect_(curx, cury, w, h, true, tags); //  rounded corners
            glEnable(GL_TEXTURE_2D);
            glEnable(GL_BLEND);
            defaultshader->set();
        }
    ...
    void rect_(float x, float y, float w, float h, bool lines = false, int tags = 0)
    {
		...
		if(!(tags&SU_ROUNDED)) {
            ...
            if(tags&SU_WARNING) glColor3ub(255, 0 , 0);
            else if(tags&SU_GREEN) glColor3ub(100, 255 , 100);
#define roundsize 5
            glVertex2f(x + roundsize, y);
            glVertex2f(x + w-roundsize, y);
            glVertex2f(x + w, y + roundsize);
            glVertex2f(x + w, y+h-roundsize);
            if(tags&SU_WARNING) glColor3ub(100, 255 , 100);
        ...
        glEnd();
        if(tags&SU_WARNING || tags&SU_ABORT)
        { //little images in the upper right corner
            
            glEnable(GL_TEXTURE_2D);
            glEnable(GL_BLEND);
            defaultshader->set();
            glColor3ub(255, 255 , 255);
            const char *wicons[] = {"packages/icons/su_warning.png", "packages/icons/su_abort.png"};
            settexture( tags&SU_WARNING ? wicons[0] : wicons[1]);
    ...
    void text_(const char *text, int x, int y, int color, bool shadow, bool force = false) 
    {
        ...
        else draw_text(text, x, y, su_howercolorr, su_howercolorg, su_howercolorb, force ? -0xFF : 0xFF);
    }
    ...
    void icon_(Texture *t, bool overlaid, int x, int y, int size, bool hit)
    {
        ...
        if(hit) glColor4ub( (float(su_howercolorr)/255.0f)*100.0f+155.0f, (float(su_howercolorg)/255.0f)*100.0f+155.0f, (float(su_howercolorb)/255.0f)*100.0f+155.0f, 255);
        else glColor3fv(color.v);
        rect_(x, y, xs, ys, 0);

        if(overlaid)
        {
            if(!overlaytex) overlaytex = texload("data/guioverlay.png", 3);
            glBindTexture(GL_TEXTURE_2D, overlaytex->id);
            glColor3fv(light.v);
            rect_(x, y, xs, ys, 0);
        }
    ...
    {		
        if(visible())
        {
            if(!slidertex) slidertex = texload("data/guislider.png", 3);
    ...
    int button_(const char *text, int color, const char *icon, bool clickable, bool center, const char *description = 0)
    {
        ...
        if(visible())
        {
			...
			if(hit&& su_howerinfo) {
				if(!howerstart)
				{
					howerx = curx;
					howery = cury;
					howerstart = totalmillis + su_howerinfodelay * 1000; //start the infobox <delay> sec after you started pointing on something
				}	
				if(description)
				{
					if(!description[0]) description = 0;
						...
						DELETEP(howerinfo)
						howerinfo = newstring(description);
					}
				}
				else DELETEP(howerinfo)
			}
			else if(curx == howerx && cury == howery) howerstart = 0; //last time this element was the howered one, but you dont point at it anymore
			if(hit && (curx != howerx || cury != howery)) howerstart = 0; //not the right one focused	
			...
			if(icon)
            {
                if(icon[0] != ' ')
                {
                    ...
                    defformatstring(tname)("packages/icons/%s%s%s%s", su_iconsdir, su_iconsdir[0] ? "/" : "", icon, ext ? "" : ".jpg");
					Texture *te = textureload(tname, 3, true, false);
					if(te == notexture) {
						formatstring(tname)("packages/icons/%s%s", icon, ext ? "" : ".jpg");
						te = textureload(tname, 3, true, false);
					}
                    icon_(te, false, x, cury, ICON_SIZE, clickable && hit);
                }
                x += ICON_SIZE;
            }
    ...
    {
        if(!skintex) skintex = texload("guiskin.png",3);
        ...
        {	
            ...
            glColor4f(j ? light.x : 1.0f, j ? light.y : 1.0f, j ? light.z : 1.0f, passes<=1 || j ? ((float)su_skinalpha/100) : ((float)su_skinalpha/200));
    ...
    void end()
    {
        ...
        {	
            ...
            if(tcurrent) 
            {
                *tcurrent = max(1, min(*tcurrent, tpos));
                extern bool setcurrenttab(int n);
                setcurrenttab(*tcurrent);
            }
        ...
        else
        {
			...
			if(howerinfo)
			{
				if(howerstart && howerstart < totalmillis)
				{
					glPushMatrix();
					
					int tw, th;
					int altscale = curfont->scale;
					curfont->scale *= float(su_howerinfoscale)*0.1f;
					text_bounds(howerinfo, tw, th);
					int abw = 0; //abstand w : difference betweeen box and selected option w
					int abh = -1*FONTH;
					
					
					//int rightborder = howerx+skinx[2]*SKIN_SCALE+ tw; //
					//float onew = rightborder * scale.x + origin.x; //if this value is not between 0 and 1 the box is outside of the screen on the right
					int altsa = su_skinalpha;
					su_skinalpha = min ( 100, su_skinalpha + 40); 
					drawskin(howerx-skinx[2]*SKIN_SCALE+abw, howery-skiny[6]*SKIN_SCALE+abh-th, tw, th, 0, 9);
					drawskin(howerx-skinx[5]*SKIN_SCALE+abw, howery-skiny[6]*SKIN_SCALE+abh-th, tw, 0, 9, 1);
					su_skinalpha = altsa; //ugly workaround, but im lazy today
					
					draw_text(howerinfo, howerx+abw, howery+abh-th);

					curfont->scale = altscale;
					glPopMatrix();
				}
			}
            glPopMatrix();
